...
def rgb_to_ascii_code(rgb):
    incs = (0x00, 0x5f, 0x87, 0xaf, 0xd7, 0xff)

    base_winner = 0
    base_err = 0
    for part in rgb:
        base_winner *= len(incs)
    ...
    non_base_errs = np.sum(np.abs(_NON_BASE - rgb), axis=1)
    non_base_winner = np.argmin(non_base_errs)
    non_base_err = non_base_errs[non_base_winner]
    if non_base_winner > 15:
        non_base_winner += -16 + 232

    return base_winner if base_err <= non_base_err else non_base_winner




def get_set_back_color_cmd(rgb):
    if rgb is -1:
        return u"\033[0m"
    if config.has_true_color_support():
        return u"\033[48;2;%d;%d;%dm" % tuple(rgb)
    elif config.has_ascii_color_support():
...
def get_set_front_color_cmd(rgb):
    if rgb is -1:
        return u"\033[0m"
    if config.has_true_color_support():
        return u"\033[38;2;%d;%d;%dm" % tuple(rgb)
    elif config.has_ascii_color_support():
        code = rgb_to_ascii_code(rgb)
